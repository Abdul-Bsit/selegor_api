...
#                         if len(unique_property_ids) >= 600:
#                             print("\n✅ 600 Properties Scraped. Stopping Scraper.")
...
#     desired_order = ['Property Name','Price (€)', 'Description', 'Address', 'Phone Number', 'Image URL','Website Name', 'Expired']
#     df = df[desired_order]
...
#     df_cleaned.to_csv("Cleaned_Seloger_Properties.csv", index=False)
#     df_cleaned.to_json("Cleaned_Seloger_Properties.json", orient="records")

#     print("\n✅ Scraping & Preprocessing Completed Successfully!")
#     return df_cleaned
...
# Load existing property_id values from CSV (if file exists)
def load_existing_property_ids():
    if os.path.exists(CSV_FILE):
        df_existing = pd.read_csv(CSV_FILE)
        return set(df_existing["Property ID"].astype(str))
    return set()
...
# Scrape listing pages and extract new property details
def scrape_all_pages():
    all_properties = []
    existing_property_ids = load_existing_property_ids()  # Load stored property IDs
    unique_property_ids = set(existing_property_ids)  # Track new ones
    current_url = LISTING_URL
    page_number = 1
    max_retries = 3
...
def preprocess_data(df):
    ...
    df["Address"] = df["Address"].str.replace(r"\s{2,}", " ", regex=True).str.strip()
    df["Property Name"] = df["Address"]
    desired_order = ['Property ID', 'Property Name', 'Price (€)', 'Description', 'Address', 'Phone Number', 'Image URL', 'Website Name', 'Expired']
    df = df[desired_order]
    print("\n✅ Data preprocessing completed successfully!")
    return df
...
def save_results(df):
    ...
    print(f"\n📂 Saved results to CSV: {CSV_FILE}")

    # Save as JSON
    df_final.to_json(JSON_FILE, orient="records", indent=4)
    print(f"\n📂 Saved results to JSON: {JSON_FILE}")

# Run scraper and save new data
def run_scraper():
    df = scrape_all_pages()
    if not df.empty:
        df_cleaned = preprocess_data(df)
        save_results(df_cleaned)
        print("\n✅ Scraping & Preprocessing Completed Successfully!")
